...
In this tutorial you will learn how to create your own custom template for category listing pages. In this example we want to customize the CMS content, replace the default product listing, extend the default product-box and add a few styling adjustments. This custom template could be used for products (or for example product streams) that should be particularly presented, like a new clothing series.
...
We will add some custom LESS/CSS modifications later. Therefore, we add a wrapping element with an individual class `custom-detail` as a container class:
...
The CMS content that can be added to a category in the backend is optional, so in order to not display an empty element if no content is added we will create a prompt that checks if the `cmsHeadline` or `cmsText` contains any information.
...
In the last step we will add styling to the product-box itself, add the hover effects and finalize the look of our listing. Notice that we are using the `.unitize()` mixin to calculate the `pixel` values into `rem` values. Notice that our product images in this example have a transparent / single color background, which allows us to simply set the background of our product-box to the same color. We are also using the `top / translate` solution to center the product image horizontally:
