...
The Shopware backend with its powerful javascript framework offers a lot tools to design nice interfaces for the end user.
This document will describe, how existing modules can be extended.

<div class="toc-list"></div>

## The general mechanism
This is a technical description of the mechanisms, which you might helpful to extend backend applications. If you want
to fast forward to the [next section](#extending).
...
As you can see, there is a database field `controller` which indicates, which controller will load the backend application.
...
inherit from `Shopware_Controllers_Backend_ExtJs`. If the module is build using the new backend components, the base
...
This is done performing another Ajax request to the configured controller. Shopware uses some minification here, usually
...
As you can see, the `load` action of the controller is used in this case. Shopware will then concatenate all requested dependencies
return them to the javascript application, which will then be able to show the backend module.

### Round up
Generally speaking, we need to remember, that a given backend controller `Customer` is linked to the `backend/customer/app.js`
 file. The `index` action of the controller will be used to load that `app.js`, the `index` action of the controller
 will be used to load all other components.

## Extending
Extending backend modules is done by injecting own javascript into Shopware's default modules. This is done using
...
### Example #2: Own components
In some cases you also want to introduce whole new components to a backend module. This example looks similar to the
first one, there are subtle changes, however. In this example, a new tab is introduced to the customer module, which
will just a simple "hello world" message - but you could easily extend it to show own data.
...
In the callback, there is a distinction between `index` and `load` action. The `index` action is the right place
 to introduce the new component, so we know, that is available before all other components are loaded. The `load` action - just
 as in the first example - is used to modify the original module, so that the new tab is actually being shown:
...
## Download
The full example can be <a href="{{ site.url }}/exampleplugins/SwagSearchBundle.zip">downloaded here</a>.
