...
the `index` controller, which will not be cached if the `price` tag is active. The suffixed `-1` indicates that the current
shop has ID `1`.
...
In this case, I added a cookie named `my-cache-cookie`. If I call the index page of the shop now, nothing has changed,
as the cookie isn't set yet. As I have the `debug` flag enabled, I will get this info:
