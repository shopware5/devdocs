...
class SeoExample52 extends \Shopware\Components\Plugin
{
    public static function getSubscribedEvents()
    {
        return [
            ...
            'Enlight_Controller_Action_PostDispatchSecure_Backend_Seo' => 'addGlossaryCount'
        ];
    }
    ...
    public function loadPerformanceExtension(\Enlight_Controller_ActionEventArgs $args)
    {
        ...
        $subject->View()->extendsTemplate('backend/performance/controller/glossary.js');
    }

    public function addGlossaryCount(\Enlight_Controller_ActionEventArgs $args)
    {
        $controller = $args->getSubject();
        $request = $controller->Request();
        $view = $controller->View();

        if ($request->getActionName() !== 'getCount') {
            return;
        }
        ...
        $counts['glossary'] = (int) $wordsCount;
