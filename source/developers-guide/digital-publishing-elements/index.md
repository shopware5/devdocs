...
In the **getSubscribedEvents()** method we return all necessary events we want to subscribe to and the corresponding callback method. At least we have three callback methods which get called by our subscriber:
...
For the next step we will look at the first event, which subscribes to the post dispatch of the backend, so we can extend the backend with our ExtJS template files in the **onPostDispatchBackend** callback. To add a new element to the Digital Publishing module we have to extend the original container.js of the editor. For this we create our own file called **extension.js**. All files go to the ```Views``` directory of our plugin in ```backend/digital_publishing_sample/view/editor```. We also create the custom handler for our element in the ```elements``` subdirectory called **youtube\_element\_handler.js**. We're adding these files to the backend template with the ```extendsTemplate()``` method.
...
Now that we created the handler we have to add it to the list of available element handlers. Therefore we created the extension.js to extend the original editor: ```Views/backend/digital_publishing_sample/view/editor/extension.js```
...
We're using an override of the ```Shopware.apps.SwagDigitalPublishing.view.editor.Container``` to add a new instance of our custom handler to the ```elementHandlers``` property, which holds a list of all available handlers. Don't forget to call ```me.callParent(arguments)``` to make sure the original component can work as intended. When you installed the plugin correctly, you should already see the new element as a selectable item in the element menu of the Digital Publishing editor. 

## Adding configuration fields
We now already have a new selectable element. Now we want to add some configuration fields, which the user can use to edit the element. Therefore we will use the ```createFormItems()``` method to create a custom fieldset.
...
It is recommended to implement at least one fieldset as shown in the example to keep the look and feel of the settings through all elements. In the items of the fieldset you can use any kind of ExtJS fields, so you have many possibilities to create your custom element. All fields of your element are just defined by this form. But there also some things you have to take care of.
<br />
<br />
**Loading existing data to the fields**<br />
The configuration form will not automatically load data from a model, because all elements are saved in a JSON encoded payload string. The decoded data of the fields is already passed in the ```data``` argument of the ```createFormItems()``` method. All you have to do is to set the value of each field manually. You then can also define a default value for the field. Example: ```value: data['maxWidth'] || 280,```.
<br />
<br />
**Adding change listeners**<br />
To always update the corresponding data model with recent data from your element configuration, you have to implement event listeners to your form fields. The listener has to be bind to the ```me.updateElementRecord``` method, which needs the form panel (```me.formPanel```) and the element model (```elementRecord```) as its arguments. In most cases you will register to the *change* event of the field. This is necessary for the live preview, to show the changes directly to the user, and some other reasons.
<br />
<br />
**Making fields translatable**<br />
We already implemented the functionality to translate the fields of an element into different languages. If you want to make some fields of your element translatable, you just have to add the corresponding property to the field: ```translatable: true```. When you do so, a small globe will appear at your field, where the user can add the translations.
...
The rest is just little bit of frontend stuff and Smarty code. All fields of the element are available in the ```{$element}``` variable.
...
The YouTube element doesn't need much styling. We're using a combination of ```width``` and ```max-width: 100%;``` to let the YouTube frame not become bigger as the design layer. So in a responsive layout the frame would shrink accordingly when there is not enough space.


## Data handling ##
Not every element has a simple structure like the YouTube sample. Sometimes you want to manipulate the data of the element before it gets passed to the frontend. Therefore we added a special event you can subscribe to. You could again use a subscriber to do so.
