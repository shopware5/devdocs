...
It has all you need, to start with development: A working `Bootstrap.php` file, an event subscriber `Frontend.php` with an
...
components might be confusing at the first look.

Running `sw plugin:create --haveFilters SwagFilterTest` you can easily create the boilerplate code with a simple example
...
This is ideal for long-running tasks or those, you want to trigger with a native cron job.

`sw plugin:create --haveCommands SwagCommandTest` will create such a command for you:
...
As in most other cases, the plugin create will figure out the name from the command by your plugin name: `SwagCommandTest`
will becomde `CommandTest` as a command class name - a more realistic example would be `SwagAboCommerce` would generate an
`AboCommerce` command.
...
## API
Its quite easy to add new resources to the Shopware API and make them available via REST. With `sw plugin:create --haveApi SwagApiTest`
this gets even simpler:
